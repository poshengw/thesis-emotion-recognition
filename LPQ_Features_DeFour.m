function   LPQResult = LPQ_Features_DeFour(I,winSize,decorr,freqestim,mode)
...
Num_Size = 2;
EachLength = fix( size(I,2)/Num_Size);
EachWidth = fix(size(I,1)/Num_Size);
LPQResult = [];


    xx=0;
    yy=0;
while xx+EachLength<= size(I,2)
   while yy+EachWidth<=size(I,1)

%% Initialize
img = I(yy+1:yy+EachWidth,xx+1:xx+EachLength);
